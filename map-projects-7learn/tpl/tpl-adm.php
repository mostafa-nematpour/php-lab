...
<head>
    ...
    <link rel="stylesheet" href="assets/css/style.css<?= "?v=" . rand(99, 9999999) ?>" />
    <style>
        body {
            background: #f2f2f2;
        }
...
<body>
    <div class="main-panel">
        ...
        <div class="box">
            ...
            <a class="statusToggle" href="?">همه</a>
            ...
            <table class="tabe-locations">
                ...
                <tbody>
                    <?php foreach ($locations as $loc) : ?>
                        <tr>
                            <td><?= $loc->title ?></td>
                            <td class="text-center"><?= Hekmatinasser\Verta\Verta::instance($loc->created_at)->format('%d %B %Y') ?></td>
        ...
        $(document).ready(function() {
            ...
            $('.statusToggle').click(function() {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    data: {
                        loc: button.attr('data-loc')
                    },
                    success: function(response) {
                        if (response == 1) {
                            button.toggleClass('active');
                            if (button.html().trim() == 'فعال') {
                                button.html('غیر فعال');
                            } else {
                                button.html('فعال');
                            }
                        }
